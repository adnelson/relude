...
")
let cons: 'a. ('a, array<'a>) => array<'a> = (x, xs) => Relude_Array_Instances.concat([x], xs)
...
")
let append: 'a. ('a, array<'a>) => array<'a> = (x, xs) => Relude_Array_Instances.concat(xs, [x])
...
")
let tail: 'a. array<'a> => option<array<'a>> = xs => {
  let l = length(xs)
  if l == 0 {
    None
  } else if l == 1 {
...
")
let tailOrEmpty: 'a. array<'a> => array<'a> = xs =>
  switch tail(xs) {
  | Some(ys) => ys
  | None => []
  }
...
")
let init: 'a. array<'a> => option<array<'a>> = xs => {
  let l = length(xs)
  if l == 0 {
    None
  } else {
...
")
let initOrEmpty: 'a. array<'a> => array<'a> = xs =>
  switch init(xs) {
  | Some(arr) => arr
  | None => []
  }
...
")
let last: 'a. array<'a> => option<'a> = xs => {
  let l = length(xs)
  if l == 0 {
    None
  } else {
...
")
let takeExactly: 'a. (int, array<'a>) => option<array<'a>> = (i, xs) =>
  if i < 0 || i > length(xs) {
    None
  } else {
...
")
let rec takeWhile: 'a. ('a => bool, array<'a>) => array<'a> = (f, xs) =>
  switch head(xs) {
  | Some(x) if f(x) => prepend(x, takeWhile(f, tailOrEmpty(xs)))
  | _ => []
  }
...
")
let dropExactly: 'a. (int, array<'a>) => option<array<'a>> = (i, xs) =>
  if i < 0 || i > length(xs) {
    None
  } else {
...
")
let rec dropWhile: 'a. ('a => bool, array<'a>) => array<'a> = (f, xs) =>
  switch head(xs) {
  | Some(x) if f(x) => dropWhile(f, tailOrEmpty(xs))
  | _ => xs
  }
...
")
let mapOption: 'a 'b. ('a => option<'b>, array<'a>) => array<'b> = (f, xs) =>
  Relude_Array_Instances.foldLeft(
    (acc, curr) =>
      Relude_Option_Base.fold(acc, v => Relude_Array_Instances.concat(acc, [v]), f(curr)),
    [],
    xs,
  )
...
")
let catOption: 'a. array<option<'a>> => array<'a> = xs => mapOption(v => v, xs)
...
")
let partition: 'a. ('a => bool, array<'a>) => (array<'a>, array<'a>) = (f, xs) =>
  Belt.Array.partition(xs, f)
...
")
let splitAt: 'a. (int, array<'a>) => option<(array<'a>, array<'a>)> = (i, xs) =>
  if i < 0 || i > length(xs) {
    None
  } else {
...
")
let prependToAll: 'a. ('a, array<'a>) => array<'a> = (delim, xs) =>
  Relude_Array_Instances.flatMap(v => [delim, v], xs)
...
")
let intersperse: 'a. ('a, array<'a>) => array<'a> = (delim, xs) =>
  switch head(xs) {
  | None => []
...
")
let replicate: 'a. (int, array<'a>) => array<'a> = (i, xs) =>
  Relude_Array_Instances.foldLeft(
...
")
let zipWith: 'a 'b 'c. (('a, 'b) => 'c, array<'a>, array<'b>) => array<'c> = (f, xs, ys) =>
  Belt.Array.zipBy(xs, ys, f)
...
")
let zipWithIndex: 'a. array<'a> => array<('a, int)> = xs =>
  zip(xs, Relude_Int.rangeAsArray(0, length(xs)))
...
")
let sortWithInt: 'a. (('a, 'a) => int, array<'a>) => array<'a> = (f, xs) =>
  Belt.SortArray.stableSortBy(xs, f)
...
")
let sort = (type a, ordA: module(ORD with type t = a), xs: array<a>): array<a> => {
...
")
let distinctBy: 'a. (('a, 'a) => bool, array<'a>) => array<'a> = (eq, xs) =>
  Relude_Array_Instances.foldLeft(
    (acc, curr) => Relude_Array_Instances.containsBy(eq, curr, acc) ? acc : append(curr, acc),
    [],
    xs,
  )
...
let removeFirstBy: 'a. (('a, 'a) => bool, 'a, array<'a>) => array<'a> = (innerEq, v, xs) =>
  Relude_Array_Instances.foldLeft(((found, ys), x) =>
    found
      ? (true, append(x, ys))
      : switch innerEq(v, x) {
        | true => (true, ys)
        | false => (false, append(x, ys))
        }
  , (false, []), xs) |> snd
...
")
let removeEachBy: 'a. (('a, 'a) => bool, 'a, array<'a>) => array<'a> = (innerEq, x, xs) =>
  Relude_Array_Instances.foldLeft((ys, y) => innerEq(x, y) ? ys : append(y, ys), [], xs)
...
")
let insertAt: 'a. (int, 'a, array<'a>) => array<'a> = (targetIndex, newX, xs) =>
  switch splitAt(targetIndex, xs) {
  | Some((before, after)) =>
    Relude_Array_Instances.concat(before, Relude_Array_Instances.concat([newX], after))
  | None => xs
  }
